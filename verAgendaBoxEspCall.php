...
$sql = "select ab_box.box_numero, ab_box.box_codigo,COUNT(*) 
from ab_box, ab_box_especialidad
where ab_box.box_codigo = ab_box_especialidad.box_codigo
and ab_box_especialidad.esp_codigo = '$ESP_CODIGO'
GROUP BY ab_box.box_numero, ab_box.box_codigo
HAVING COUNT(*) > 0
order by ab_box.box_codigo ASC
";
...
function mostrar($tableContent, $box_codigo){
$link=conexion();
/**
...
if ($row_count != 0){
 ...
 json_encode($blocks);
	...
	$blocks[] = array('id' => null, 'rut' => null, 'name' => null, 'apellido' => null, 'estado' => null, 'nameDos' => null, 'day' => null, 'startTime' => null,
			'endTime' => null);
	 json_encode($blocks);
	 echo "<center> <h3> <font size ='3', color ='red'> No existe horario para esta esp. </font> </h3> </center>";
    ...
    for ($j = 2; $j < 8; $j++) {
        ...
        foreach ( $blocks as $block ) {
            ...
            if ( ( $block["day"] == $j ) && ( $block["startTime"] == $i ) ) {
                ...
                $blockID    = $block["id"];
               $rowContent .= "\t" . "<td class='forcedWidth' style='line-height:8px; width:1000px; height:100%;' bgcolor='$lala'  rowspan='$rowspan'   data-id='$blockID' class='block-cell'> <a href='verBloqueCall.php?PTD_RUT=$blockID'> <font color='$lele'> $content </br> $contentTres </br> $contentNueve </br> $contentDiez   </font> </a>  </td>" . "\r\n";
            ...
            <!-- Optionally, you can add icons to the links -->
			 ...
			 <li class="treeview active">
              ...
              <ul class="treeview-menu">
				...
				<li class="active"><a  href="boxVerPorEspCall.php"> Ver Boxs Por Esp. </a></li>
      ...
      <div class="content-wrapper">
		...
		<div class="box">
          <div class="box-body">
		   ...
		   $queryNombreEsp= sqlsrv_query($link, "select * from especialidad where especialidad.ESP_CODIGO = '$ESP_CODIGO'");
  //$nmyestudiante = sqlsrv_num_rows($estudiante);
  $rowNombreEsp = sqlsrv_fetch_array($queryNombreEsp);
  $emmNombreEsp = $rowNombreEsp['ESP_NOMBRE'];
		  
		  ?>
		  
		   <center> <h3> Agenda Ocupación Médica Box </h3> </center>
		  <center> <h4> especialidad : <?php echo $emmNombreEsp; ?> </h4> </center>
			   ...
			   <?php
					...
					<td><center>  </center></td>
					<td><center>  </center></td>
					<td><center> <font color='red' > Especialidad sin horarios en box </font> </center></td>
					<td><center>  </center></td>
					<td><center>  </center></td>
					<td><center>  </center></td>
					</tr>
		   ...
		   $queryDatosBox= sqlsrv_query($link, "select * from ab_box where ab_box.box_codigo = '$box_codigo'");
  //$nmyestudiante = sqlsrv_num_rows($estudiante);
  $rowDatosBox = sqlsrv_fetch_array($queryDatosBox);
  $acb_codigo_actividad = $rowDatosBox['acb_codigo_actividad'];
  $box_numero = $rowDatosBox['box_numero'];
						  ...
						  if($acb_codigo_actividad=='1'){
							...
							if($contador==0)								
								...
								echo "<tr>";
											...
											//$sqlSiete = "select * from ab_box_especialidad, especialidad where ab_box_especialidad.esp_codigo = especialidad.esp_codigo and  ab_box_especialidad.box_codigo = '$box_codigo'";
...
HAVING COUNT(*) > 0";
						...
						echo "</tbody>";
								...
								mostrar($tableContent,$box_codigo);
								echo " </div>";			
